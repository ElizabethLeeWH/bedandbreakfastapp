...
public class BookingsRepository {
	...
	public void newUser(User user) throws BookingException {
		int updatedRows = template.update(SQL_INSERT_NEW_USER, user.email(), user.name());
		if (updatedRows<1) {
			throw new BookingException("Failed to insert new user");
		}
		;
	}
	...
	public void newBookings(Bookings bookings) throws BookingException {
		int updatedRows = template.update(SQL_INSERT_NEW_BOOKING, bookings.getBookingId(), bookings.getListingId(),
				bookings.getDuration(), bookings.getEmail());
		if(updatedRows<1){
			throw new BookingException("Failed to insert new booking");
		}
	}
}
