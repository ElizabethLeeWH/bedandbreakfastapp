...
import vttp2023.batch4.paf.assessment.repositories.BookingException;
...
public class ListingsService {
	...
	public void createBooking(Bookings booking) throws BookingException {
		...
		if(userOpt.isPresent()){
			...
			System.out.println("Successfully booked for existing user");
		} else {
			User newUser = new User(booking.getEmail(), booking.getName());
			bookingsRepo.newUser(newUser);
			System.out.println("User successfully created"+ booking.getEmail());
			bookingsRepo.newBookings(booking);
			System.out.println("Booked successfully for" + booking.getName());
		}
	}
